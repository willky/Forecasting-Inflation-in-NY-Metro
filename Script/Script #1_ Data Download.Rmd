---
title: "NY Metro CPI Data Download"
author: "Levy Will"
date: "`r Sys.Date()`"
...
## CPI Data Download with BLS API {.tabset .tabset-fade .tabset-pills}
...
For this project, I downloaded monthly data for all 61 items BLS publishes data for pertaining to the NY Metro Area. Since the BLS API only allows a maximum of 50 variables or items per API request, the download process is broken into two parts - first 50 variables and last 11 variables. 

This data used project only uses data from 2003 through 2022, as BLS limits data downloads via API to 20 years per request. In the future, the project may be expanded to include data prior to 2003.
...
## Merge the Two Dataframes {.tabset .tabset-fade .tabset-pills}

After downloading the two data sets, I merge them into one big data, which will be used for the analysis. 
